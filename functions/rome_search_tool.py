"""
title: Rome Search Tool
description: LLM-invokable medical literature search. Dispatches to the rome search API or ZeroEntropy depending on RETRIEVAL_BACKEND, with signed GCS URLs for citations.
...
class Tools:
    """Open WebUI Tool that searches medical literature on demand."""
    ...
    async def search_medical_literature(
        ...
        """Search the medical literature collection.
        ...
        try:
            ...
            if is_rome:
                ...
                snippets = await search_zeroentropy(self.valves, query, k)
                ...
                await __event_emitter__(
                    {
                        "type": "status",
                        "data": {
                            "description": f"Found {len(snippets)} passages",
                            "done": True,
                        },
                    }
                )
